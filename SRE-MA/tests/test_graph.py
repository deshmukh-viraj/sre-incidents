...
# 1. Trigger a scenario that causes ambiguous metrics
# (Or just manually send a webhook with fake "novel" data)
...
payload = {
    ...
    "alerts": [{
        "status": "firing",
        "labels": {
            # THIS ALERT NAME DOES NOT EXIST IN YOUR ROUTING.PY
            "alertname": "ServiceBehaviourAnomaly", 
            "severity": "critical",
            "service": "payment_gateway",
            # NO RUNBOOK LABEL
        },
        "annotations": {
            "summary": "Payment gateway experiencing intermittent timeouts with no upstream degradation"
        }
    }]
}

# Send the webhook
response = httpx.post("http://localhost:8080/webhook/alert", json=payload)
